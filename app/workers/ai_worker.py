...
class AIWorkerPool:
    ...
    def _process(self, task_id):
        ...
        if 'CONCLUSION' in analysis_types:
            prompt_parts.append(
                '''CONCLUSION（总结）：请生成一份内容充分、观点清晰、适合直接阅读的 Markdown 长文，而不是简单摘要或流水账。参考以下输出风格和结构，但章节标题必须根据原文真实内容动态生成，不要机械套用“赚钱”示例：

这段内容的核心，其实可以浓缩成一句话：

> **用一句有判断力的话概括作者真正想表达的核心冲突、标准或结论。**

### 一、主题化的观点标题

每个主题使用“### 一、”“### 二、”这样的 Markdown 标题。建议生成 6-12 个主题，数量由原文信息量决定。每个主题都必须包含：
- 先用一段话解释观点，不要只写结论；
- 说明作者为什么这样判断；
- 明确指出原文中的案例、事实或论据，以及它服务于哪个观点；
- 在段末给出清晰的判断或结论；
- 原文有直接、强烈或具有争议性的表达时，可以用 Markdown 引用保留，例如：> **原文观点**；不得为了客观而弱化。

对于原文明确列举的行为、条件或方法，使用 Markdown 列表；不要把多个独立观点挤成一段。对于“不是不要做某事，而是要把它服务于结果”这类边界，要明确写出对比关系。

在主题章节之后继续输出：

## 最终可以提炼成一个“动态生成的逻辑”

用“目标 → 问题 → 原因 → 判断标准 → 行动 → 结果”的箭头链条重新组织全文，并用 1-3 段解释每个环节如何衔接。不要补充原文没有的商业建议或事实。

### 一句话总结

用一句非常简洁、有传播力但不改变原意的话收束全文。

总结要求：去除“啊、嗯、对吧、然后”等口语废话；修正明显的语音识别错误；不要按照原文时间顺序复述；不要只罗列观点而缺少解释；区分作者观点、作者引用的案例和客观事实；只使用原文明确表达或可以直接推出的内容；最终让没有看过原视频的人也能快速理解作者的思维逻辑。'''
            )
        prompt_parts.append('\n原文：\n' + text)
        prompt = '\n\n'.join(prompt_parts)
