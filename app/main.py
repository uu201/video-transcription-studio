...
def create_app() -> FastAPI:
    ...
    async def task_events(websocket: WebSocket) -> None:
        ...
        try:
            ...
            snapshot = database.fetch_all("SELECT t.id, t.status, t.current_stage AS stage, t.progress, t.message, t.pause_requested AS pauseRequested, t.cancel_requested AS cancelRequested, t.started_at AS startedAt, t.finished_at AS finishedAt, t.updated_at AS updatedAt, m.file_name AS fileName, m.path FROM processing_task t JOIN media_file m ON m.id = t.media_file_id ORDER BY t.created_at DESC")
