...
class ProcessingTaskRepository:
    ...
    def list(self):
        """按创建时间倒序读取任务。"""
        return self.database.fetch_all("SELECT t.*, m.file_name, m.path FROM processing_task t JOIN media_file m ON m.id=t.media_file_id ORDER BY t.created_at DESC")
